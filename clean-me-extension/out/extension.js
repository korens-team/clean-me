...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('extension.helloWorld', () => {
        ...
        function updateDecorations(ranges) {
            if (!editor) {
                return;
            }
            const changeDecoOpt = [];
        ...
        var temp = 2;
        editor.edit(builder => builder.replace(textRange, editedText)).then(function () {
            deltas.forEach((index) => {
                textRangeArr.push({ range: new vscode.Range(index.start - 1 + temp, editor.document.lineAt(index.start - 1 + temp).range.start.character, index.end - 1 + temp, editor.document.lineAt(index.end - 1 + temp).range.end.character), description: index.description });
                temp += 3;
            });
