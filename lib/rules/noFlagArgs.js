...
class noFlagArgs {
  ...
  static apply(syntaxTree) {
    ...
    this.functions.forEach(f => {
      ...
      syntaxTree = estraverse.replace(syntaxTree, {
        enter: function (node) {
          if (node.type === "FunctionDeclaration" && f.node.id.name === node.id.name && node.loc !== null) {
            this.remove();
          }
        }
      });
  ...
  static findFlags(node) {
    let flags = [];
    let params = node.params; // there is only if under the function declaration
    ...
    if (node.body.body.length <= 1) {
      estraverse.traverse(node, {
        enter: subnode => {
          ...
          if (node.body.body.length <= 1 && (subnode.type === 'IfStatement' && subnode.test.type === 'Identifier' && params.find(param => param.name === subnode.test.name) || subnode.type === 'IfStatement' && subnode.test.type === 'UnaryExpression' && subnode.test.argument.type === "Identifier" && params.find(param => param.name === subnode.test.argument.name))) {
            ...
            if (!this.isVariableBeingUsedUnderNode(subnode.test.name || subnode.test.argument.name, subnode.consequent)) {
              flags.push({
                ...
                flaggedPartOriginalParamsIndexes: params.map((par, index) => {
                  if (this.isVariableBeingUsedUnderNode(par.name, subnode.consequent)) {
                    return index;
                  }
                }),
                unflaggedPartOriginalParamsIndexes: params.map((par, index) => {
                  if (this.isVariableBeingUsedUnderNode(par.name, subnode.alternate)) {
                    return index;
                  }
                })
              });
